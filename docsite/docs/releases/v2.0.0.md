...
- **Debian Bullseye EOL**: Bullseye stopped receiving security updates in 2024 and is slated to be EOL. The binaries are now built with Debian Bookworm which uses openssl v3,
...
This prevents unintended major version upgrades when using auto updaters. Use `:2` to stay up to date
